...
An Audiobookshelf podcast library SHALL use the shared hero-on-left presentation when it meets the wide geometry conditions and selected-row replacement otherwise. In hero-on-left, the selected podcast's cover, metadata, and downloaded-episode workspace SHALL occupy the left pane while the single-column podcast-show browser occupies the right rail. In the replacement presentation, the same selected-show detail (title, author, description, cover) SHALL replace the active podcast-show row in list flow. The podcast tab SHALL obtain placement from the shared arrangement and SHALL NOT define a separate fallback.
...
- **THEN** it uses hero-on-left when wide geometry fits and inline selected-show detail (title, author, description, cover) otherwise
