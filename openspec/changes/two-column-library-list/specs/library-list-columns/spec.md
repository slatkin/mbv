...
The Power View library list SHALL choose its column count from the width available to the list pane itself, not from the terminal width, and SHALL use at most two columns. Two columns SHALL be used only when every cell would still meet a minimum readable width. When the available width falls below that threshold the list SHALL render a single column.

#### Scenario: Wide list pane

- **WHEN** the library list pane is wide enough for two cells at the minimum cell width plus the inter-column gap
- **THEN** the list SHALL render two columns of items

#### Scenario: Narrow list pane

- **WHEN** the library list pane is not wide enough for two cells at the minimum cell width
...
- **WHEN** the queue column is widened, narrowed, or collapsed, changing the width left over for the list pane
