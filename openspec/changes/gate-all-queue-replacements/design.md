...
```rust
pub(super) enum ReplacementExecutor { Routed, Pending }
```
Make the slot `pending_queue_replacement: Option<(PendingQueueAction, ReplacementExecutor)>`. A `Routed` payload is always a `PlayItems`. The executor is a type rather than a flag, so the confirm arm can't pick the wrong one.
...
- `Routed`: destructure `PlayItems { items, start_idx, source, .. }` and call `play_items_routed`.
