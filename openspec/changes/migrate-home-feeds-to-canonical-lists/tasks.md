...
- [ ] 2.2 Compose persistent `InlineMediaBrowser` and `WideMediaList` controls for the applicable Home paths, reworking the in-tree `173bdba1`/`400c0b59` wiring. REMOVE (not just avoid) the existing parent-owned `cursor`/`scroll` mirror fields on `HomeComponent`, the legacy `render_home_content` underpaint, and the `set_scroll(offset)` render-offset write-back. Keep the active control authoritative: ordinary refresh preserves target and locally clamps without parent cursor/scroll input; breakpoint or discrete navigation transitions perform one `ViewportAnchor` handoff. Movement and context-menu anchoring read control-exported geometry. Add no per-section cursor cache or App-wide interaction mirror.
- [ ] 2.3 Remove Home destination list duplication — including the legacy underpaint and render-offset write-back introduced by `173bdba1`/`400c0b59` — and retain existing parent-owned effects, overlays, and typed message translation. Leave the bespoke `*HitRegion` path wired and untouched — do not remove it as "duplication"; `restore-mouse-support` (#638) owns its migration. Add no new mouse wiring.
