...
- [ ] 5.8 Reclaim the removed Selector row's space: when a destination presents no Selector row/pill bar (Continue), the panel beneath SHALL occupy the freed row(s) — no blank spacer band; update the surface-conformance spacer assertion and verify Narrow/Wide (`cargo nextest run -p mbv`).
