...
- [ ] 5.4 Fix the destination Latest population regression: Emby flat libraries (Movies/Home Videos/Generic) show an EMPTY Latest; TV Latest/Upcoming must refresh after playback stop. Restore a destination-owned Latest fetch/population path independent of Home; verify Movies and Home Videos Latest rows populate and `library_browse_actions::tv_latest_tests::refresh_after_stop_reloads_selected_tv_mode` passes (`cargo nextest run -p mbv`). REOPENED 2026-09-23: the `54ddffcd` fix only fired on the library-open branch; a restored/selected library issued no `get_latest` (empty Latest in the live app). Correction in flight.
