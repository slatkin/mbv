...
- [ ] 5.7 Add the missing end-to-end mounted-tick test for the Emby flat Latest new-content marker (has_new → `•` on the pill; selecting Latest acknowledges and clears it; a later async refresh does not restore it), closing the coverage gap found by the functional review (`cargo nextest run -p mbv`).
