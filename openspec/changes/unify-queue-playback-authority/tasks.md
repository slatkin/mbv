...
- [~] 3.1 Centralize Bound-queue mutation, observed active slot, and transition coordination in owner state used by Bare, Local daemon, and packaged `mbvd` paths; verify each owner kind reaches the same mutation functions rather than mutating a Client snapshot. (daemon half done `48fcee16`; Bare half = §4)
