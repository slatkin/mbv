...
The mounted parent subscribes to mouse events and owns `MouseGestureState`, recognizing gestures and parent-owned pills/workspace/overlays. The child populates and resolves its own `HitRegions<Target>` during view. The parent delegates point resolution and translates results to destination-specific typed messages. No global map, second router, child subscription, or duplicate row coordinate path exists. Keyboard precedence remains solely in `router.rs`/`key_policy.rs`.

### D5 — Composition and visual proof

The slice composes generic Emby, Movies, Emby homevideos/podcast libraries, narrow TV Series browsing, and Wide TV's right rail. Non-hero two-column browsers remain unchanged. `render_plain_rows` is re-homed/parameterized first with characterization output preserved. Visual correction and user live confirmation happen before any UI test changes; then focused rendered tests and representative metadata/state fixtures are added. Each migrated surface provides one-painter evidence and Wide/Narrow manual evidence. No bespoke exception is permitted without an umbrella design update.
