...
This is planning for one slice, not implementation. It depends on the `restore-mouse-support` mouse delivery/gesture spine being landed (merged on `feat/migrate-tui-to-tuirealm`) before this slice's mouse wiring, and on the independent Feed prerequisite where applicable, and targets `feat/migrate-tui-to-tuirealm`; it must remain a distinct PR/rollback boundary from PR #606 and other slices. Split `browser.rs` and `tv_workspace.rs` ownership-preservingly before or with wiring and enforce the 800-line gate.
