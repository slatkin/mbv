...
closed one-column presentation names. **Grid presentation** is the accepted
name for the existing non-hero two-column catalog arrangement over the shared
owner; it is not a new provider or destination. The existing `two-column list`
...
| Grouped Music tracks | `music_workspace.rs`; `render/components/music_workspace.rs` | Wide track table | Provider workspace / filtered track view | N/A | `music_workspace_actions_tests.rs`, `music_workspace_cursor_tests.rs`; `render/tests_music_wide.rs`, `tests_music_wide.rs` |
...
| Audiobookshelf Podcast shows | `src/app/components/audiobookshelf_podcast.rs`; `render/components/audiobookshelf_podcast.rs` | Wide show rail | Inline show replacement | N/A | `audiobookshelf_podcast_component_tests.rs`; `render/tests_audiobookshelf_podcasts.rs`, `tests_audiobookshelf_podcasts.rs` |
| Audiobookshelf Podcast filtered episodes | `audiobookshelf_podcast.rs`; `render/components/audiobookshelf_podcast.rs` | Wide episode workspace | Normal/Narrow filtered episode workspace | N/A | `audiobookshelf_podcast_component_tests.rs`; `render/tests_audiobookshelf_podcasts.rs`, `tests_audiobookshelf_podcasts.rs` |
| Audiobookshelf Books | `src/app/components/audiobookshelf_book.rs`; `render/components/audiobookshelf_book.rs` | Wide book rail | Inline book replacement | N/A | `audiobookshelf_book_component_tests.rs`; `render/tests_audiobookshelf_books.rs`, `tests_audiobookshelf_books.rs` |
| Audiobookshelf chapter/audio-part rows | `audiobookshelf_book.rs`; `render/components/audiobookshelf_book.rs` | Wide chapter workspace | Normal/Narrow chapter workspace | N/A | `audiobookshelf_book_component_tests.rs`; `render/tests_audiobookshelf_books.rs`, `tests_audiobookshelf_books.rs` |
