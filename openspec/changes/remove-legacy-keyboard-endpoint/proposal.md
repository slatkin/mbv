...
- Resolve the shared-globals crux (`q`, Tab/BackTab, `1`–`9`, `.`) and the destination-independent Alt-key path, and replace the ephemeral prompt gates (`skip_intro_end_ticks`/`next_up_item`) and the per-key playback gate (including the 300ms double-tap that falls through on the first press) with real TuiRealm state and subscriptions.
