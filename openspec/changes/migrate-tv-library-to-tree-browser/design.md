...
Extend the tree's input vocabulary with plain structural heading and spacer entries, separate from selectable `TreeNode<Target>`. Interleave headings before root shows and spacers between consecutive groups in one settled visible row flow, preserving TV's current grouped-list separators. Structural rows have no destination target, exposed tree-library node handle, or parent/child edges. The shared cursor, paging, filter, viewport, painting, and point resolution use that same flow; structural rows are skipped by selection, marks, hit resolution, and effects. Stripe and heading appearance follow the shared visual contract confirmed against the rendered surface: the heading itself paints unstriped, spacers paint unstriped, and zebra striping resets at each heading (a row's stripe derives from its position within its group, not its absolute flow or viewport position, and holds stable under clipping as well as scrolling, never running continuously across a group boundary), a heading sits on the depth-zero title baseline via the shared two-column content inset, a played row reads the existing muted/played palette role above the depth colours, and ordinary node titles fall back by depth through the cream, yellow, and aqua roles. Reconciliation validates node targets/edges atomically and also invalidates retained geometry when heading content changes. Without headings the old Music projection and pixels remain unchanged.
...
Show activation retains current show/Workspace behavior, season activation toggles expansion, and episode activation emits the same episode playback request as the Hero Workspace. `Right` follows the shared TreeBrowser operation: on an expanded show root it activates the show detail, on an expanded descendant row it is a no-op, and on an unexpanded expandable row it expands it. Navigation uses `apply(TreeOperation)` transitions and selected stable target rather than translating back through the former flat-series cursor. Keep context/playback effects resolved from the selected target's Emby snapshot; headings return no target. Preserve the single central keyboard router and Library Panel's latest-frame mouse eligibility and hit geometry. TV Inline Search remains independent from the shared tree filter session.
