...
Hero placement is still Service- and surface-dependent: Audiobookshelf and Feeds retain separate detail blocks, Home retains one in its narrow presentation, and some Emby surfaces do not adopt hero-on-left when wide. This contradicts the live inline-detail requirements and leaves an obsolete arrangement, fallback, border variant, and vocabulary for later UI work to preserve.

## What Changes

- Make hero placement invariant across every hero-bearing browse surface: hero-on-left when wide enough, otherwise selected-row replacement with inline detail.
- Preserve the existing minimum-height guard for hero-on-left; when it cannot fit, use selected-row replacement rather than a separate top block.
...
- Remove the retired top arrangement, geometry helper, border variant, fallback behavior, tests, comments, and domain vocabulary rather than retaining a dormant compatibility path.
- Add an ADR recording hero-on-left and inline hero as the only supported hero placements, and update `CONTEXT.md` to remove the conflicting retired term.
...
- `library-list-hero`: Standardize selected-row replacement, suppression, geometry ownership, and parent activation below the wide geometry threshold.
- `library-list-columns`: Remove retired-placement-specific column rules; hero-on-left browsers are one column and inline presentations use one column.
- `music-library-hero`: Replace grouped Music's narrow separate fallback with its inline selected-album detail.
...
- `audiobookshelf-book-browsing`: Replace the book tab's narrow separate fallback with inline selected-book detail while retaining its wide left workspace.
