...
The now-playing row SHALL show no elapsed time: its duration slot is empty. It SHALL project the `NowPlaying` semantic state with live progress following the playback position; the live percentage renders in the right-aligned slot, replacing the duration entirely, preceded by the current braille throbber glyph. When runtime is unknown the row SHALL project no `progress_percent` and show the throbber alone. Non-active rows SHALL keep progress where it is today (inline trailing badge) with the duration slot unchanged.
