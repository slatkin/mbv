...
| `dispatch/queue.rs` `execute_pending_queue_action` PlayItems | non-autostart goes to the owner idle load |
| `dispatch/queue.rs` ClearQueue | also sends `player.clear_queue()` |
| `dispatch/queue.rs` `save_queue_as_playlist` | reads owner lineage into `owner_queue_lineage` |
...
(`shell/run.rs`) and its two test callers call `restore_queue_state`
...
- [queue.rs is 793 lines.] → Net change there is negative (the
