...
- [x] 1.1 Resolve the drag target by step direction in `MediaList::drag_selection_into_window`
...
- [x] 1.2 Reconcile the owner tests that pinned the nearest-row landing
...
- [x] 1.3 Add the named guard test that is Invariant 6's enforcement point: a step that drags never parks
...
- [x] 2.1 Update the per-surface drag-at-edge assertions to the leading-edge landing: Emby library, Music
...
- [x] 2.2 Update the grouped-list tick-integration viewport-step evidence
...
- [x] 3.1 Confirm the standing artifacts name the leading-edge rule and the recurrence history:
...
- [x] 4.1 Full gates: `cargo fmt --all -- --check`; `cargo clippy --workspace --all-targets -- -D warnings`;
