...
- [x] 1.1 In `src/app/key_policy.rs`, change the `panel_mode_cycle_x` entry's
...
- [x] 1.2 Add or extend a `key_policy`/`router` test asserting that with
...
- [x] 2.1 Delete the `Library,` variant from `enum ComponentId` in
...
- [x] 2.2 Fix the stale doc comment in `src/app/shell_library.rs:12-14`: it
...
- [x] 2.3 Run `rtk cargo clippy --workspace --all-targets` and
...
- [x] 3.1 Rewrite row 64 of `docs/architecture/interactive-surface-ledger.md`
...
- [x] 3.2 Leave the `| Library | ...` rows (65–72, 89) unchanged — "Library"
...
- [x] 4.1 `rtk cargo check -p mbv`, `rtk cargo nextest run -p mbv`,
      `rtk cargo clippy --workspace --all-targets`, `rtk ast-grep scan`,
      `rtk cargo fmt --all -- --check` — all green.
- [x] 4.2 `openspec validate resolve-library-parent-phantom --strict` passes.
- [x] 4.3 Manual check (or a shell-level test if one already exercises the
