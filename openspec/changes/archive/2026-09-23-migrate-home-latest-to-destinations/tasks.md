...
- [x] 3.2 Add Latest to Feeds' group/filter selector using only the already loaded combined entries; verify played entries remain visible, selector exit restores the former group/filter, `w` leaves Latest and cycles the restored filter once, no fetch on entry, and `r` refresh updates Latest (`cargo nextest run -p mbv`).
- [x] 3.3 Project shared title parts, provider dates, marker glyph and selected detail through existing row/Hero painters for non-TV Latest; verify focused buffer tests at Narrow and Wide (`cargo nextest run -p mbv`).
...
- [x] 5.3 Manually inspect Home and every eligible destination (including a Music tree round-trip) at Narrow and Wide, confirm Latest selectors/dates/markers with real loaded content, and record the observations before requesting user visual sign-off; do not archive before that sign-off. OBSERVED 2026-09-23 (user, all-good): Continue tab labelled `Continue` with no pill bar and the panels beneath reclaiming its row; Movies/Home Videos/Generic Latest populate with provider dates and the `•` marker clearing on select; TV Latest/Upcoming unchanged; podcast and Feeds Latest present with dates; Music has no Latest pill and its group/tree round-trip works; no Home Latest anywhere; Narrow and Wide both confirmed.
- [x] 5.4 Fix the destination Latest population regression: Emby flat libraries (Movies/Home Videos/Generic) show an EMPTY Latest; TV Latest/Upcoming must refresh after playback stop. Restore a destination-owned Latest fetch/population path independent of Home; verify Movies and Home Videos Latest rows populate and `library_browse_actions::tv_latest_tests::refresh_after_stop_reloads_selected_tv_mode` passes (`cargo nextest run -p mbv`). REOPENED 2026-09-23: the `54ddffcd` fix only fired on the library-open branch; a restored/selected library issued no `get_latest` (empty Latest in the live app). Correction `f3dfea33` committed + unit-reviewed PASS; full suite green (2240/2240); acceptance pending user live confirmation.
- [x] 5.5 Present Home as Continue: the tab SHALL be labelled `Continue` and the now-empty Selector row/pill bar SHALL be removed; update the Home render characterization and mouse-hover tests; verify (`cargo nextest run -p mbv`). Re-run the 5.1 gate suite after 5.4 and 5.5.
- [x] 5.6 Remove Music Latest: Music is NOT a Latest-eligible destination (user decision 2026-09-23; invalidates accepted row 2.4). Remove Music's Latest selector arm and flat-Latest path, restore Music's pre-2.4 selector/list behaviour, update Music tests, and narrow `destination-latest-modes` eligibility to exclude Music (`cargo nextest run -p mbv`).
- [x] 5.7 Add the missing end-to-end mounted-tick test for the Emby flat Latest new-content marker (has_new → `•` on the pill; selecting Latest acknowledges and clears it; a later async refresh does not restore it), closing the coverage gap found by the functional review (`cargo nextest run -p mbv`).
- [x] 5.8 Reclaim the removed Selector row's space: when a destination presents no Selector row/pill bar (Continue), the panel beneath SHALL occupy the freed row(s) — no blank spacer band; update the surface-conformance spacer assertion and verify Narrow/Wide (`cargo nextest run -p mbv`).
