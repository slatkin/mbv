...
- [x] 2.3 Correct selected/active/played geometry at the real owners: `feeds.rs::render_feeds_content` keeps the Wide rail one column, paints the hero/detail-pane title exactly once, and aligns edge selection/active markers to full-width rows; `feed_row.rs::render_feed_entry_cell` fills the whole `cell_w` with one contiguous background and honors the caller's `show_title` without extra suppression.
...
- [x] 3.4 Land this as an independent docs/implementation slice before `migrate-home-feeds-to-canonical-lists`; do not fold #634/#637, Music/Audiobookshelf canonical-slice work, or PR #606 sequencing.
...
> Historical ordering note (do not copy): this prerequisite was delivered test-first — buffer/geometry fixtures were written alongside the render correction, with user live visual confirmation recorded only at acceptance above. That ordering is recorded as historical fact and is not precedent. Future canonical Feeds work follows whichever campaign workflow is in force when that work is done; do not rewrite the completed task history above to claim any later workflow was followed.
