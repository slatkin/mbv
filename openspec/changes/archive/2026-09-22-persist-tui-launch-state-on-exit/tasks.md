...
- [x] 2.2 Implement the same bounded query for Feeds, Audiobookshelf podcast, and Audiobookshelf book destinations, using Feed/show/book identities rather than labels or row indices; extend the narrowest existing component tests for filter/group/show/bucket and selected-item extraction, then run the matching `cargo nextest run -p mbv` test filters.
- [x] 2.3 Assemble exactly the selected tab's destination snapshot during orderly teardown, include Panel focus, exclude Queue selection and every unselected destination, and verify with an App/shell teardown test that reads the saved file without constructing live externals.
...
- [x] 3.3 Restore Library/Queue Panel focus after the selected destination is ready without passing a Queue target; add one real `Application::tick()` integration test proving Queue receives focus while its selected item follows normal Queue initialization, then run that test filter.
