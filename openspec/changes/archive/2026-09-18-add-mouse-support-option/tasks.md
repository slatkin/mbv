...
- [x] 2.2 Update the `init_terminal` call in `Model::run` (src/app/shell_run.rs) to pass `self.app`'s config `mouse_support` value. Verify: manual run with `mouse_support = false` shows no mouse capture; default launch unchanged. (User manual verification: pass.)
