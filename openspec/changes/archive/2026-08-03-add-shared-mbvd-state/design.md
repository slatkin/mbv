...
### Restrict TCP to loopback/private networks and permit optional TLS

Shared-data TCP endpoints are restricted to loopback and private-network addresses; WAN endpoints are rejected. Plaintext TCP is allowed within that explicit LAN-only boundary so operators do not need to provision certificates for every internal endpoint. TLS remains available as an optional defense in depth for deployments that want it, using normal hostname and trust-chain validation when configured. Unix-domain endpoints may omit TLS because credentials and data do not leave the host and filesystem permissions protect the socket.

WAN access was rejected because the handshake carries a reusable Emby bearer token and the documents reveal viewing and routing history. Certificate pinning and generated self-signed identities were rejected for the initial design because they add a second trust-distribution mechanism; operators may use optional TLS with a private CA if additional LAN protection is desired.

For `tls://` endpoints, the TLS handshake completes before the client serializes or sends its Emby token. Connection and authentication have hard time bounds and happen outside database and playback locks.
