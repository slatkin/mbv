...
- [x] 1.1 Add `DaemonEvent::QueueEnriched(Vec<EmbyItem>)` (or equivalent name)
...
- [x] 1.2 In `daemon_control.rs`'s `CtrlCmd::UnifiedAdoptQueue` handler
...
- [x] 1.3 Handle the new `DaemonEvent` in `daemon_run.rs`'s event loop: call
...
- [x] 1.4 Add a daemon-side unit test (mocked `EmbyClient`/`Player`, no live
...
- [x] 2.1 Remove the `spawn_enrich_queue_state` call from the
...
- [x] 2.2 Decide whether `LocalDaemonBootstrap.positions`
...
- [x] 2.3 Update or remove any test in `src/app/tests_daemon_bootstrap.rs`
...
- [x] 3.1 Add a short cross-reference in
...
- [x] 4.1 Full workspace check: `cargo fmt --all`, `cargo clippy --workspace
...
- [x] 5.1 `MediaSemanticState::from_progress`
...
- [x] 5.2 Monotonic enrichment merge: in `PlaybackQueue::merge_fetched_slot`'s
...
- [x] 5.3 `get_continue_watching` (`crates/mbv-core/src/api_client_library.rs:189`):
