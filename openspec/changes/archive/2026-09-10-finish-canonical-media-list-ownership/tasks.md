...
- [x] 9.1 After sections 2-8 are complete, run a final review pass over every included destination.
  - Result: spec-axis PASS (all seven destinations meet the ownership endpoint; deferred TV inline-search item resolved UNREACHABLE with evidence — the rail is never consulted while inline search is active and the guard is documented load-bearing; no cross-destination drift; podcast delta conformance confirmed); standards-axis reported 14 items, all within the 9.2 scope (12 docs-only, 2 code-touching).
- [x] 9.2 Correct only the stale comments, contradictory Wide-orientation scenario titles, compatibility paths, and narrow structural enforcement still judged necessary in that review; verify no destination implementation is folded into reconciliation.
  - Result: commit `701cc635` — 12 docs-only fixes (stale mirror/lockstep/legacy comments, browser-left/hero-right orientation wording across layout.rs/browser/feeds/book/tests) + `left_row_map` compat disposition (legacy grid path preserved with truthful comments; no behavior change). Optional ast-grep left_* enforcement skipped (registration fought the codebase; optional per review). No destination implementation folded in.
- [x] 9.3 Run final focused and project gates required by the accepted changes, including formatting, `cargo check -p mbv`, relevant `cargo nextest run -p mbv`, clippy, `ast-grep scan`, and the file-size gate; record any unrelated failure without marking this campaign complete.
  - Result: all gates pass — fmt, cargo check, nextest 1510/1510, clippy (only pre-existing queue.rs:491 needless-return), ast-grep scan, and the pre-push-stage file-size gate AFTER the user-approved extraction fix `72a62d4e` (browser/mod.rs 713, browser_component_tests.rs 675, new browser/state.rs 123 + browser_mouse_tests.rs 168; all governed files ≤800). All eight OpenSpec validations pass.
- [x] 9.4 Verify this ledger and GitHub issue #681 agree on every row and completion state; archive this umbrella and close #681 only after final OpenSpec validation succeeds and no included row remains open.
