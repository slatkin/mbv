...
- [x] 1.1 Bump `CTRL_PROTOCOL_VERSION` from `10` to `11` in
  `crates/mbv-core/src/ctrl.rs`; verify with `cargo check -p mbv-core`.

## 2. Core type change

- [x] 2.1 In `crates/mbv-core/src/player/types.rs`, change
  ...
  mention.
- [x] 2.2 In `crates/mbv-core/src/daemon/core.rs`, change
  `PendingIdleQueueLoad.stopped_run` from
  `(PlaybackRequestId, PlaybackGeneration)` to `PlaybackGeneration`.
- [x] 2.3 In `crates/mbv-core/src/daemon/run.rs`, change `PlaybackRunIdentity`
...
- [x] 3.1 Fix `run_identity` construction/assignment in
  ...
  `PlaybackGeneration`.
- [x] 3.2 Fix the `run_identity` field type in
...
- [x] 5.1 Fix `run_identity`/`stopped_run` literals in
...
- [x] 5.2 Fix `run_identity: (0, 0)` literals in
...
- [x] 5.3 Verify with `cargo nextest run -p mbv-core` and
...
- [x] 6.1 Run `cargo fmt`, `cargo clippy --workspace --all-targets -- -D
