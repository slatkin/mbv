...
> Historical ordering note (do not copy): this prerequisite was delivered test-first — buffer/geometry fixtures were written alongside the render correction, with user live visual confirmation recorded only at acceptance above. That ordering predates the visual-first rule now in force. Future canonical Feeds work MUST obtain explicit user live visual approval before any UI buffer/geometry test is added or changed; do not cite this change as precedent for test-first UI work.
