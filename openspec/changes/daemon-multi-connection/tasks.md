...
- [ ] 1.1 Write new ADR (0012) that supersedes ADR 0003 (exclusive ctrl connection) and documents the multi-connection model
- [ ] 1.2 Update ADR 0007 (control authority) to reflect: authority returns on next ctrl command (not on connect), connect does NOT override EmbyRemote, authority clears only on last client disconnect
