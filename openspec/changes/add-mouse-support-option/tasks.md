...
- [ ] 4.2 Full gates: `cargo fmt`, `cargo clippy --workspace --all-targets -- -D warnings`, `cargo nextest run -p mbv-core`, `cargo nextest run -p mbv`, and `cargo test --release -- --test-threads=4` for the mbv package (CI parity). Verify: all green, no new warnings.
