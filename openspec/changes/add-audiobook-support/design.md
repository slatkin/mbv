...
### Author-surname grouping via `human_name`, computed once at catalog build
Decision 4: `Name::parse(name).map(|n| n.surname().to_owned()).unwrap_or(name)`, first-listed author only for multi-author books. Store `author_display` (raw credit) and `author_sort_key` (surname) per book at catalog-build time, mirroring `music_group.rs::build_grouped_album_catalog`'s existing split between display and sort key, rather than re-parsing on every render.
...
- [Merged-timeline mpv projection is unproven for Audiobookshelf's authenticated per-file URLs] → Verify against a real multi-file book early in implementation (see Open Questions); if per-entry headers aren't supported, fall back to `edl://` before committing to the approach elsewhere.
...
- Does mpv's `loadfile` accept the same Bearer `Authorization` header across a multi-file playlist, or does per-entry-header playback require `edl://` URL syntax instead? Single server/token per book makes either workable; resolve by testing against a real multi-file Audiobookshelf book before finalizing the source-preparation code in `crates/mbv-core`. Doesn't change the spec (`audiobookshelf-book-playback` already states the externally observable contract, not the URL mechanism) or the task breakdown's shape.
