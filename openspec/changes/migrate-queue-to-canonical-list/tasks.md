...
- [ ] 1.1 Confirm accepted HEAD `7e908d1`, PR #606 feature-branch stacking, and canonical foundation availability; record the distinct Queue rollback boundary.
- [ ] 1.2 Characterize existing Queue rendering and interactions before UI test edits using metadata-bearing Local and Remote fixtures: title, focus, active progress, reorder affordances, scope controls, and remote state.
- [ ] 1.3 Characterize supported Wide/Normal and narrow/mini geometry, scrolling, selection, images, active row, and one-painter execution path; identify the legacy Queue body painter to remove or bypass.
...
- [ ] 2.3 Preserve Queue parent/shell ownership of Local/Remote scope and controls, reorder, playback, title, Player/queue authority, persistence, and active-state policy; carry resolved targets/indices through typed messages where effects are required.
...
- [ ] 3.2 Perform visual correction at supported Wide/Normal and narrow/mini widths, including metadata, active progress, Local/Remote scope, reorder, remote state, focus, scrolling, and images; obtain explicit user live confirmation before test changes.
...
- [ ] 4.1 After explicit user live approval, add/update focused render and geometry tests with metadata-, active-progress-, focus-, scope-, reorder-, remote-, image-, refresh-, and target/scroll-bearing fixtures; cover progress bounds and target retention.
