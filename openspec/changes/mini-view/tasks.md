...
- [ ] 4.2 Update `render/tests_queue.rs` (and any other test asserting the
      old unfocused queue-only styling) to expect focused styling when the
      queue holds focus, at both narrow and wide widths.
- [ ] 4.3 Add a render test confirming mini view starts at library-only by
      default (fresh app, narrow terminal, no prior interaction).

## 5. Docs

- [ ] 5.1 Update `openspec/specs/panel-mode/spec.md` is handled by
