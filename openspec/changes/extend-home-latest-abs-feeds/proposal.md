...
- Widen `ShelfEntryWire`/`AudiobookshelfShelf` parsing to capture the embedded `media.metadata` and `recentEpisode` fields the live server actually returns, instead of discarding them down to bare IDs.
