...
- [ ] 3.1 Extend existing mounted `Application::tick()` tests to verify F3 focus, selection and pointer delivery through shell synchronization and identity-bearing activation after a refreshed snapshot; check narrow and ordinary widths and missing-target handling (`cargo nextest run -p mbv -E 'test(tick_integration)'`).
