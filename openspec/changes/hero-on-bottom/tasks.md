...
- [x] 1.2 Reuse the existing `hero_height_for_width` function and its
...
- [x] 1.3 Reserve `hero_area` at the bottom of `content_area` and pass
...
- [x] 1.4 Update `layout.left_area` to be `list_area` (the row
...
- [x] 2.1 In `render_power_list`, after the list has rendered, call
      ...
      change needed.
- [x] 2.2 Verify the hero paints at the bottom of the content area,
...
- [x] 2.4 Confirm the hero always shows the cursor's item regardless
...
- [x] 3.3 Remove the `DisplayRow::Hero` variant from
...
- [x] 4.1 In `render_power_list`, the `hero_area` height calc (task
...
- [x] 4.2 Confirm the bottom hero handles Series items via
...
- [x] 6.1 `src/app/render/list_tests.rs` has 4 tests today (item
...
- [x] 6.2 Add a new test enforcing the 1-col/2-col parity invariant:
...
- [x] 6.3 Add a new test: `hero_paints_below_list_area_in_two_column_mode`.
...
- [x] 6.4 Add a new test: `hero_height_is_constant_above_the_image_cap`.
...
- [x] 6.5 Add a new test: `selected_cell_uses_carat_and_double_hash_in_two_column_mode`.
      ...
      edge of the cell.
- [x] 6.6 Add a new test: `hero_content_tracks_cursor_when_selection_scrolled_offscreen`.
