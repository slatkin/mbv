...
The mounted Music/Audiobookshelf parent owns mouse subscription and gesture recognition per the `restore-mouse-support` spine (an accepted prerequisite slice that must merge first); this slice does not add or modify subscription or gesture-recognition code. The embedded child owns render-derived hit regions and resolves list points; explicit child targets precede parent workspace targets, and any duplicate list hit path is removed. Each reachable breakpoint has one body painter, proven by source trace and a focused execution counter/assertion where practical.
