...
- [x] 4.1 Add the Browser-pane Wide stripe regression at the Render Component layer: a real
  ...
  (Accepted: impl `ff5e97e0` as `library_wide_browser_stripes_items_not_structural_rows`;
  Standards+Spec conformant, pairs NOT swapped, parity vector discriminates item-only counting.)
- [x] 4.2 Add the Workspace-arm counterpart asserting the `LibraryPanel` pair, plus one assertion per
  ...
  (Accepted: impl `ff5e97e0` as `library_wide_workspace_stripes_with_library_panel_pair` with both
  stripe≠own-body guards; dual review conformant.)
- [x] 4.3 Add the accent regressions on the library path: focused selected row → bold focus-accent
  ...
  (Accepted: impl `ff5e97e0` as `library_wide_accent_keeps_selected_stripe_and_unfocused_rows_plain`;
  Item-scoped scan confirmed.)
- [x] 4.4 Convert `non_adjacent_multi_selected_rows_and_unfocused_cursor_paint_selected_surface`
  ...
  (Accepted: impl `ff5e97e0`, correction `b65764ed` renamed it to
  `..._take_accent_with_own_parity` name-only; remaining wrapper uses geometry/scroll only.)
- [x] 4.5 Correct the `SelectedRowOnQueueColumn` and `SelectedRowOnLibraryPane` rows of the coverage
  ...
  (Accepted: impl `ff5e97e0`, correction `b65764ed` — Spec BLOCK-CLEARED via focused re-review;
  adjacent dangling cites also fixed, `yes` claims grounded to real probes or `—` residuals.)
