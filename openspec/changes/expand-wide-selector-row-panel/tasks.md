...
- [ ] 4.1 Remove `ListControls` and the `controls` field on `LibraryPanelContent` (`content.rs`), `paint_list_controls_row` (`slots.rs`), and their re-exports in `library_panel/mod.rs` (the `ListControls` import at `mod.rs:29`, the `paint_list_controls_row` re-export at `mod.rs:47`).
