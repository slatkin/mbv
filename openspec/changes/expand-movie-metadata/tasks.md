...
- [x] 4.2 Manual visual sweep in a real terminal: Wide Movies with and without an overview, with and
...
- [x] 6.1 Select credits as every person in the item's `people` list — every `Director` first in
...
- [x] 6.2 Separator under the overview text: a line of `▁` (U+2581) block characters spanning the
  ...
  overview.
- [x] 6.3 Make the overview box's content interactively scrollable when it exceeds the box's height:
  ...
  per AGENTS.md).
- [x] 6.4 Zebra-stripe the credits table rows: alternate rows (by table position, first row
...
- [x] 6.5 Cast names in yellow: the credits table's name column renders in a semantic text role
  ...
  unstriped rows.
- [x] 6.6 Wheel delivery over the hero box must work in a real terminal: diagnose why a real
...
- [x] 6.7 Scroll scope narrows (user direction 2026-09-15): on overflow ONLY the cast and crew table
