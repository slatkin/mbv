## 1. Prove the escape-cell path

- [ ] 1.1 Scratch-paint one forced-width OSC 8 label cell into the Wide Hero pane through
  `Frame::buffer_mut()` (`set_symbol` on a cell carrying `\x1b]8;;<url>\x1b\\<label>\x1b]8;;\x1b\\` plus
  `CellDiffOption::ForcedWidth`), run the TUI, ctrl-click the label, then delete the scratch code.
  Verify: the URL opens in the browser on a supporting terminal (kitty/foot/iTerm2/WezTerm/Windows
  Terminal); the diff walk does not mis-place the following text; the scratch hunks are gone from
  `git status`; the observed outcome and terminal are recorded in `design.md` under Risks. If it does
  not hold, stop and report before continuing — the fallback (hero-pane hit geometry) changes the specs.

## 2. Emby data model (mbv-core)

- [ ] 2.1 Add `EmbyPerson { name, role, kind }` and `EmbyLink { name, url }`, replace `EmbyItem.genre`
...
- [ ] 2.2 Parse the new fields in `parse_item` (`Genres`, `People` Name/Role/Type with `Role` optional,
...
- [ ] 2.3 Add `ExternalUrls` to the `Fields` query string in `api_client_library.rs` and
...
- [ ] 2.4 Update the `EmbyItem { .. }` literals in `crates/` test modules for the new fields. Verify:
  `cargo nextest run -p mbv-core` green.
- [ ] 2.5 Update the `EmbyItem { .. }` literals in `src/` test modules for the new fields. Verify:
  `cargo nextest run -p mbv` green with no behavior change.

## 3. Movie hero content (producer)

- [ ] 3.1 Emit the Movie genre row (every genre, joined) and the provider-link row, and carry the links
...
- [ ] 3.2 Select the credits in the Movie branch of `hero_content_emby`: every `Director` in provider
...
- [ ] 3.3 Carry `credits` on `HeroContent` through the owner's content building and confirm the Narrow
...
## 4. Wide Hero box, credits table and clickable links

- [ ] 4.1 Extract the overview Main content box painting and its tests out of `hero_header.rs` into a
...
- [ ] 4.2 Render the box when the item has overview text or credits, and paint the credits table one
...
- [ ] 4.3 Layout the table: name column sized from the longest rendered name, one shared second column,
...
- [ ] 4.4 Paint clickable link labels: sanitize (`http`/`https`, no control bytes), gate on declared
...
- [ ] 5.1 Run `cargo fmt`, `cargo clippy --workspace --all-targets -- -D warnings`, and
  `cargo nextest run -p mbv -p mbv-core`. Verify: all green.
- [ ] 5.2 Manual visual sweep in a real terminal: Wide Movies with and without an overview, with and
  ...
  at both breakpoints, nothing overflows a pane, no escape bytes appear as visible text.

## 6. Documentation and archive

- [ ] 6.1 Record the new concepts in `CONTEXT.md` (the provider-link row and the credits table) and
...
- [ ] 6.2 `openspec validate expand-movie-metadata --strict`. Verify: passes.
- [ ] 6.3 At archive, sync the deltas into `openspec/specs/library-panel/spec.md`. Verify:
