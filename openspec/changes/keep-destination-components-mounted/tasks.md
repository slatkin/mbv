...
- [x] 1.1 Add `Model::live_destination_keys(&self) -> HashSet<ComponentId>` (or
...
- [x] 1.2 Add `Model::reconcile_destination_mounts(&mut self)` in the same file:
...
- [x] 1.3 Call `self.reconcile_destination_mounts()` once per tick in
...
- [x] 2.1 In `src/app/shell_browser.rs::mount_emby_browser`, remove the
...
- [x] 2.2 Test in `src/app/shell_browser_tests.rs`: build a two-Emby-library
...
- [x] 2.3 Test: switch away from library A, mutate A's item list in `app.libs`,
...
- [x] 3.1 Apply the task-2.1 transformation to `sync_tv_workspace`
...
- [x] 3.2 Same for `sync_music_workspace` (`src/app/shell_music_workspace.rs:49-64`).
...
- [x] 3.3 Same for `sync_audiobookshelf_book` and `sync_audiobookshelf_podcast`
...
- [x] 3.4 Test: no two destination components share a `library_id` after a
...
- [x] 4.1 Confirm `sync_active_destination` (`src/app/shell_library.rs:23-41`)
      is the only site calling `application.active()` for a destination
      child. `rg -n "\.active\(" src/app/shell_*.rs` should show it only in
      `sync_active_destination`, `sync_queue`, the overlay/modal open paths,
      and the intentional transient `InlineSearch` activation in
      `open_inline_search` (`shell_inline_search.rs` — the inline search owns
      focus while open, a user-initiated modal, not a destination-family
      sync/mount path) — and NOT in any `sync_<workspace>` /
      `mount_emby_browser`. Verify: the grep result matches that expectation.
- [x] 4.2 Extend the existing focus tests (`shell_library.rs` `#[cfg(test)]`
...
- [x] 4.3 Test: a mounted-but-inactive destination component paints nothing
      (its `render_*_component` early-returns because the `*_id` pointer is
      `None`). Verify: `rtk cargo nextest run -p mbv`.

## 5. Ledger + final gate

- [x] 5.1 Update the Notes cells of the destination rows in
...
- [x] 5.2 `rtk cargo check -p mbv`, `rtk cargo nextest run -p mbv`,
      ...
      all green.
- [x] 5.3 `openspec validate keep-destination-components-mounted --strict` passes.
