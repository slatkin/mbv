...
When a settled grouped snapshot is replaced, the system SHALL preserve the current artist-root or album-leaf selection by stable identity when that node remains present. It SHALL preserve surviving artist expansion and multi-selection state and retain the closest practical viewport position around the selected node. Artist roots SHALL remain stable grouping and action targets across the replacement.
