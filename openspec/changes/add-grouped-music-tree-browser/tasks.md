...
- [ ] 2.2 Implement one tree state owner for selected node, expansion, viewport, marks, and geometry reconciliation across settled refresh and responsive geometry changes; verify selected-node, expansion, multi-selection, and viewport continuity with component-level `cargo nextest run -p mbv` cases.
...
- [ ] 6.1 Add the Emby client operation for Audio items by stable artist ID plus the shell-owned artist-detail cache and request identity keyed by Library destination, Service setup generation, artist ID, and settled revision; verify request construction, cache reuse, and stale completion rejection with hermetic `mbv-core` and `mbv` tests.
