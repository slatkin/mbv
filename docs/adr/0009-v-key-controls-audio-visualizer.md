...
The embedded surface is backed by a supervised PipeWire capture worker. PipeWire reads the default system output; enabling it does not reroute mpv or modify the persistent system audio graph. The preference persists in mbv's UI preferences, but PipeWire capture is only started for active local playback, never for remote or audio-pipe playback.
