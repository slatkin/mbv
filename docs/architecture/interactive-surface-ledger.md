...
| Library | Home | component (2026-08-25) | `HomeComponent` (`src/app/components/home.rs`) with Model-owned Home content and semantic section preference; shell owns fetch/result delivery, restore/persistence, and effect boundaries | `App.home`/`home_loading`/Home preference fields and `sync_home` are deleted; `push_home_content` is event-driven, `save_prefs` preserves shell-owned `home_section`, and CW/section state has no App owner; content/state/preference ownership is complete, but Home still falls through to the legacy `App::handle_key` Home catch-all and `LegacyInput`/`CONTEXT_STACK` teardown has not landed, so the row stays `component` until group 5 teardown; `rtk cargo nextest run -p mbv` (1152 passed); `rtk ast-grep scan` retains 69 pre-existing diagnostics; `rtk cargo fmt --all -- --check` |
...
| Overlay stack | Settings sidebar and setup forms | component (2026-08-24) | `SettingsComponent` (`src/app/components/settings.rs`); shell mounts `OverlayId::Settings`, mirrors App display/service state, owns panel hit geometry, and routes setup effects as typed `Msg::Service` | Local settings cursor, service cursor, setup drafts, and mouse geometry; App-free render seam; first-frame setup remains before Remote Service startup; framework teardown pending group 5 |
...
| Overlay stack | Playlists sidebar | component (2026-08-24) | `PlaylistsComponent` (`src/app/components/playlists.rs`); shell mirrors playlist snapshots and handles typed playlist effects | Variable-row geometry and mouse interaction are component-owned for painting/hit testing; duplicated `input_mouse_panels.rs` router removed; App mirror teardown pending; `rtk cargo nextest run -p mbv playlists`; component geometry and existing render tests |
...
| Library | Inline album-track interaction | component (2026-08-24) | `MusicWorkspaceComponent` (`src/app/components/music_workspace.rs`); shell pushes cached tracks and wide/narrow focus enablement; inline track focus is component-owned `track_cursor` (5.3d Album track focus unit) | Child track cursor/scroll and album-vs-track focus coupling; explicit unfocused narrow mode; App teardown pending group 5; `rtk cargo nextest run -p mbv album_track`; component local-state test and Music shell sync; `rtk ast-grep scan` |
