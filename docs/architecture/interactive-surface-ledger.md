...
| --- | --- | --- | --- | --- |
| Tab panel | Tab selection and overflow state | `TabPanel` | Library column only; owns tab hit regions | tick integration + component tests |
| Library panel | Library focus, destination owner map, selector/list/hero/workspace slots | `LibraryPanel`; Home, Emby, TV, Music, Audiobookshelf and Feeds owners supply typed content | Wide or Narrow skeleton; owns all library geometry and slot hits | tick integration at both presentations |
