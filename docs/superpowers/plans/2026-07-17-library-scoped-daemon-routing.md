...
8. **`--connect-daemon` vs. `daemon_routes."*"` precedence → moot.** The user has separately directed removal of the legacy `--connect-daemon` flag entirely, filed as its own issue, **#230**, and wired as a native GitHub blocking dependency of this issue (#223) rather than folded into #222's now-closed plan — see `docs/superpowers/plans/2026-07-19-remove-connect-daemon-flag.md`. Once #230 lands, there is nothing for the wildcard route to conflict with, and no code change is needed in this plan for that reason. #230 is a hard prerequisite (blocks #223), not a same-window race, so Tasks 8-9 here should not start until #230 is merged.
