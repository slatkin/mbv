...
pub(super) fn is_clocked_audio_error(error: &libmpv2::Error, audio_device_configured: bool) -> bool {
...
/// instead.
pub(super) fn is_superseded_jump_end_file(
...
impl PlaybackRun {
    ...
    pub(super) fn emit_track_changed(
        &mut self,
        slot_id: QueueSlotId,
    ...
    pub(super) fn on_time_pos(&mut self, pos_secs: f64, mpv: &Mpv) {
    ...
    pub(super) fn on_playlist_pos_changed(&mut self, pos: i64, mpv_pos_ticks: i64) {
        if self.active_file {
            return;
        }
    ...
    pub(super) fn on_playlist_count_changed(&mut self, count: usize) {
        if self.active_file {
            return;
        }
    ...
    pub(super) fn settle_idle_jump_on_restart(
        &mut self,
        position_ticks: i64,
    ...
    pub(super) fn on_playback_restart(&mut self, mpv: &Mpv) {
    ...
    pub(super) fn on_mpv_error(&mut self, error: libmpv2::Error, progress: &mut ProgressGuard) -> bool {
    ...
    pub(super) fn on_end_file_standalone(&mut self, reason: EndFileReason, progress: &mut ProgressGuard) -> bool {
    ...
    pub(super) fn on_shutdown(&mut self, progress: &mut ProgressGuard) {
...
pub(super) fn provider_lifecycle_close_pos(
    item: &QueueItem,
    natural_end: bool,
    runtime: i64,
    last_valid_pos: i64,
