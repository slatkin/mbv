...
// Task 5.3 – shutdown finalization: a final is_finished=true update (e.g.
// fired by the daemon on natural EOF or explicit shutdown) broadcasts the
// completion to every capable client.
#[test]
fn shutdown_finalization_broadcasts_final_is_finished_event() {
    let registry = Arc::new(Mutex::new(CtrlClients::default()));
    let (_capable_id, capable_rx) = connect_client(&mut registry.lock().unwrap());
    let mut queue = abs_queue_with_slot();
    ...
    // In-progress update.
    apply_audiobookshelf_progress(
        progress_update(1, 50.0, false),
        Some(SetupGeneration::new(1)),
        &mut queue,
        &registry,
    );
    let _ = recv_event(&capable_rx); // drain

    // Final finalization update (daemon shutdown / natural EOF path).
    apply_audiobookshelf_progress(
        progress_update(1, 100.0, true),
        Some(SetupGeneration::new(1)),
        &mut queue,
    ...
    match recv_event(&capable_rx) {
        CtrlEvent::AudiobookshelfProgress(ev) => {
            assert!(
                ev.is_finished,
                "shutdown finalization must broadcast is_finished=true"
            );
            assert_eq!(ev.setup_generation, 1);
        }
        _ => panic!("expected final AudiobookshelfProgress event"),
    }

    let ep = queue.slots()[0].item.as_audiobookshelf().unwrap();
    assert!(
        ep.is_finished,
        "slot must be marked finished after finalization"
    );
}
