...
#[test]
fn end_file_stop_with_no_jump_in_flight_adopts_mpv_entry_it_walked_off_to() {
    // Regression for fbda6981: a `Stop` EndFile with no forced_slot_id looks
    // identical whether it is debris from a superseded jump or mpv abandoning
    // the active entry on its own — only mpv's actual playlist-pos tells them
    // apart. This drives on_end_file with mpv genuinely diverged (walked off
    // idx 1 onto idx 2 with nothing asking it to) and asserts the real
    // abandonment path is taken: adopt mpv's entry, not silently drop the
    // EndFile as debris.
    let (mut session, status, events) = make_queue_session_for_pos_tests_with_events(1);
    session.pending_initial_playlist_layout = false;
    let abandoned_slot = session.active_slot_id().expect("active slot at end-file");
    let adopted_slot = session.slot_id_at(2).expect("slot at index 2");
    let mpv = test_mpv();
    mpv.command("loadfile", &["av://lavfi:sine=frequency=1000", "replace"])
        .unwrap();
    mpv.command("loadfile", &["av://lavfi:sine=frequency=1000", "append"])
        .unwrap();
    mpv.command("loadfile", &["av://lavfi:sine=frequency=1000", "append"])
        .unwrap();
    assert_eq!(mpv.get_property::<i64>("playlist-count").unwrap(), 3);
    // mpv's core only starts playing entry 0 once it activates, some time
    // after the "replace" loadfile call returns; a playlist-pos write before
    // that lands is silently superseded when the core comes up. Wait for it
    // to actually be playing before jumping mpv to entry 2 on its own.
    for _ in 0..40 {
        if mpv.get_property::<f64>("time-pos").is_ok() {
            break;
        }
        thread::sleep(Duration::from_millis(50));
    }
    mpv.set_property("playlist-pos", 2i64).unwrap();
    // Wait for mpv to actually start playing the jumped-to entry, and stay
    // there for several consecutive reads — both that it landed on index 2
    // and that time-pos has advanced meaningfully past zero, so
    // adopt_mpv_entry sees a live position (in ticks) rather than a value
    // that rounds down to a momentary 0 or a one-off transient IPC hiccup.
    let mut settled = 0;
    for _ in 0..80 {
        if mpv.get_property::<i64>("playlist-pos").unwrap_or(-1) == 2
            && mpv.get_property::<f64>("time-pos").unwrap_or(0.0) > 0.05
        {
            settled += 1;
            if settled >= 3 {
                break;
    ...
    assert_eq!(mpv.get_property::<i64>("playlist-pos").unwrap(), 2);
    let mut progress = noop_progress();

    let keep_running = session.on_end_file(mpv_end_file_reason::Stop, &mpv, &mut progress);

    assert!(
        keep_running,
        "adoption keeps the run alive; it does not signal run() to return"
    );
    assert_eq!(
        session.current_idx, 2,
        "the run's ordinal follows mpv's actual entry, not current_idx+1 or a dropped no-op"
    );
    assert_eq!(session.active_slot_id(), Some(adopted_slot));
    assert_eq!(status.lock().unwrap().current_idx, 2);
    // mpv keeps playing during the call, so an exact tick match would race;
    // the load-bearing fact is that adoption picked up mpv's own advancing
    // position rather than keeping the abandoned entry's (0).
    assert!(
        session.last_valid_pos > 0,
        "last_valid_pos follows mpv's live position in the adopted entry, not the abandoned entry's"
    );

    let events: Vec<_> = events.try_iter().collect();
    let stopped_abandoned = events.iter().any(|event| {
        matches!(
            event,
            PlayerEvent::TrackCompleted { slot_id, played, consume, .. }
                if *slot_id == abandoned_slot && !played && !consume
        )
    });
    assert!(
        stopped_abandoned,
        "the abandoned entry is reported stopped, not silently dropped as debris"
    );
    let changed_to_adopted = events.iter().any(|event| {
        matches!(
            event,
            PlayerEvent::TrackChanged { slot_id, transition }
                if *slot_id == adopted_slot && transition.is_none()
        )
    });
    assert!(
        changed_to_adopted,
        "the adopted entry is announced as an observation (no request identity)"
    );
}
