pub fn run_with_options(client: EmbyClient, audio_only: bool, hooks: DaemonRuntimeHooks) -> ! {
    ...
    // feature without affecting daemon playback.
    {
        ...
        if shared_config.shared_data_enabled {
            match crate::config::validate_shared_data_config(&shared_config) {
                Err(error) => {
                    log::error!(
                        target: "shared_data",
                        "shared-data configuration rejected; playback remains operational: {error}"
                    );
                }
                Ok(()) => match crate::shared_store::open_shared_db() {
                    Ok(db) => {
                        let store = crate::shared_worker::spawn_shared_store_worker(Arc::new(
                            Mutex::new(db),
                        ));
