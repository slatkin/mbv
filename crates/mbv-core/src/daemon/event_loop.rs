...
pub(crate) struct DaemonLoop {
    pub(crate) owner: DaemonPlayerOwner,
    pub(crate) player: Player,
    pub(crate) shared_queue: SharedQueueState,
    pub(crate) ctrl_clients: ClientRegistry,
    pub(crate) client: Arc<Mutex<EmbyClient>>,
    pub(crate) emby_runtime: Option<EmbyOwnerContext>,
    pub(crate) audiobookshelf_runtime: Option<AudiobookshelfOwnerContext>,
    pub(crate) merged_tx: mpsc::Sender<DaemonEvent>,
    pub(crate) ws_send_tx: Option<crate::ws::WsSender>,
    pub(crate) direct_commands: Vec<String>,
    pub(crate) stay_alive: bool,
    pub(crate) role: DaemonRole,
    pub(crate) audio_only: bool,
    pub(crate) last_keepalive: Instant,
    pub(crate) last_capabilities: Instant,
    /// Injected owner-queue persistence, so tests can record snapshots instead
    /// of writing real state files.
    pub(crate) store: OwnerQueueStore,
}
...
impl DaemonLoop {
    ...
    pub(crate) fn on_recv_timeout(&mut self, now: Instant) {
    ...
    pub(crate) fn handle_event(&mut self, ev: DaemonEvent) -> LoopFlow {
