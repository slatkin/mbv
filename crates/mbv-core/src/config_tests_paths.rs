...
#[test]
fn save_queue_state_reports_rename_failure() {
    let _guard = TestStateDirGuard::new();
    let path = queue_state_path();
    ...
    // First save succeeds to establish a snapshot
    assert!(save_queue_state(&state).is_ok());

    // Remove write permission on the parent directory to force rename failure
    if let Some(dir) = path.parent() {
        let original_mode = std::fs::metadata(dir).unwrap().permissions();
        std::fs::set_permissions(dir, std::fs::Permissions::from_mode(0o555)).unwrap();

        let result = save_queue_state(&state);
        std::fs::set_permissions(dir, original_mode).unwrap();

        assert!(
            result.is_err(),
            "rename into read-only directory should fail"
        );
    }
}
