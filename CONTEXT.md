...
Its resize controls are keyboard-only: `Shift+Left`/`Shift+Right`, active only while Power View itself is active, regardless of which Power View sub-panel has focus; outside Power View those keys are ignored silently rather than acting as global shortcuts that invisibly change a later Power View layout from another tab. Mouse resizing is not part of this interaction.
