...
Albums reached through configured `music.levels` are folder-like album containers whose own Emby `Primary` image is not reliable. Resolve their art through the image worker's synthetic `AudioChild` type: keep the album container ID as the request and cache identity, query Emby for its first `Audio` child, and fetch that child's `Primary` image. Cache keys remain album-based (plus the surface suffix), so every track selection shares stable album art and consumers such as MPRIS can find it. Standard Library album rows/detail, Home/Power album cards, and Power inline album detail must all preserve this rule; `src/app/images.rs` owns the `AudioChild` lookup.
_Avoid_: requesting the album container's `Primary` image directly, deriving the image source from sampled or cached track metadata, or changing the cache key to a track ID. A direct folder `Primary` request may be used only as an explicit fallback after `AudioChild`, as in Standard Library folder rendering.
...
The left column in Power View: the column that contains the Power View queue card at the top and a dedicated queue-list panel below it. The queue title/device label and Local/Remote scope pills sit above the card/panel content, not inside the queue-list panel. The queue-list panel fills the remaining left-column height and frames the usable queue viewport with the same selected-media colored-block treatment used by the Movies selected-media block; `LayoutPower::queue_area` describes only the usable queue list viewport inside that frame, excluding borders and optional decorative padding. This is the "left panel" meant by #111's user-resizable panel-width work. Its width is stored as a persistent absolute column count, not a terminal-width percentage or ratio. The default and minimum width are both 40 columns, preserving today's layout until the user explicitly widens it; restored preferences above 60% of the current terminal width are normalized down and saved so the right-side main content remains usable, except that the 40-column minimum always wins on very narrow terminals.
The column can be collapsed for the current app session with lowercase `h` while Power View is active. Collapsed means the whole physical column disappears: Power View queue card, queue title/scope pills, and queue list are not rendered, and the library side expands to the full terminal width. Collapsing while the queue side has focus moves focus to the library side; reopening keeps that library-side focus instead of restoring queue focus. The collapsed/expanded state is not persisted across restarts, and the configured width is preserved for reopening.
