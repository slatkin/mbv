...
**Queue card**:
The large artwork card shown above the queue. It represents the active playback slot when playback is active, otherwise the selected slot in the currently visible queue; it is not a library preview surface.
...
**Configured-music album image resolution**:
Albums reached through configured `music.levels` are folder-like album containers whose own Emby `Primary` image is not reliable. Resolve their art through the image worker's synthetic `AudioChild` type: keep the album container ID as the request and cache identity, query Emby for its first `Audio` child, and fetch that child's `Primary` image. Cache keys remain album-based (plus the surface suffix), so every track selection shares stable album art and consumers such as MPRIS can find it. Library rows/detail, Home album cards, and inline album detail must all preserve this rule; `src/app/images.rs` owns the `AudioChild` lookup.
_Avoid_: requesting the album container's `Primary` image directly, deriving the image source from sampled or cached track metadata, or changing the cache key to a track ID. A direct folder `Primary` request may be used only as an explicit fallback after `AudioChild`.

**Display order**:
The order items actually appear on screen in a library list, whenever that differs from API order — e.g. albums resorted by resolved artist in a grouped album view, or items resorted by `effective_sort_str` for letter-bucket grouping. Published as `left_sorted_indices` so keyboard and mouse navigation stay consistent with what's rendered, regardless of which view produced the reordering.
_Avoid_: "sorted order" — ambiguous with API order, which is also a sort result. Also avoid treating this as specific to grouped album views — any library list that reorders for display produces one.
...
**Library position**:
The per-library restoration target for returning to a library: the library's drill-down path, the active selector/pill group when that path has one, then the last focused item within that group or level. The position persists across app restarts so browsing resumes where the user left off, rather than repeatedly biasing discovery toward the top of alphabetically sorted lists; viewport scroll is secondary and may be clamped to keep that focused item visible. If saved content no longer exists, restore the deepest valid path prefix and focus the nearest sensible fallback without treating the stale position as a user-facing error. Lazy restore should avoid a root-first flash or delayed jump: a library with saved position should enter a restoring/loading state until the restored path or fallback is ready to render.
Manual refresh/rescan is an explicit reset boundary: clear that library's saved position on the user-requested reload.
_Avoid_: reducing this to raw scroll offset, global tab index, search state, detail-panel state, album track-selection focus, or one shared cursor across all libraries; "sticky" means restart-persistent in this context, not merely preserved while the process is running. (Before #361, this was tracked as two isolated scopes — one per view. Power View became the only view, so that split no longer exists; an existing on-disk position from either scope is discarded on upgrade rather than migrated, per #361.)

**Grouped album view**:
A display mode for an album listing that inserts an artist header row at each artist boundary, built from resolved artist rather than API order. One of several library list modes that produce a display order (another being the letter-grouped view, which buckets by first letter instead of artist).
...
When `music.levels` has at least one ancestor level and ends in `album`, each configured music library eagerly builds an in-memory index of leaf albums and their configured ancestor paths on a background worker. Search matches by album or ancestor label against this local album-only corpus; activating a match reconstructs the normal album destination. Explicit library refresh rebuilds the relevant index, including one coalesced replacement when refresh overlaps an in-flight build.
_Avoid_: applying this behavior to absent or album-only `music.levels`, indexing tracks, traversing the server while the user types, persisting the index, or polling.
...
**Queue column**:
The column that contains the queue card at the top and a dedicated queue-list panel below it. The queue title/device label and Local/Remote scope pills sit above the card/panel content, not inside the queue-list panel. The queue-list panel fills the remaining column height and frames the usable queue viewport with the same selected-media colored-block treatment used by the Movies selected-media block; `LayoutMain::queue_area` describes only the usable queue list viewport inside that frame, excluding borders and optional decorative padding. This is the "left panel" meant by #111's user-resizable panel-width work. Its width is stored as a persistent absolute column count, not a terminal-width percentage or ratio. The default and minimum width are both 40 columns, preserving today's layout until the user explicitly widens it; restored preferences above 60% of the current terminal width are normalized down and saved so the library side remains usable, except that the 40-column minimum always wins on very narrow terminals.
The column can be collapsed for the current app session with lowercase `h`. Collapsed means the whole physical column disappears: queue card, queue title/scope pills, and queue list are not rendered, and the library side expands to the full terminal width. Collapsing while the queue side has focus moves focus to the library side; reopening keeps that library-side focus instead of restoring queue focus. The collapsed/expanded state is not persisted across restarts, and the configured width is preserved for reopening.
Its resize controls are keyboard-only: `Shift+Left`/`Shift+Right`, active only while the column is expanded, regardless of which sub-panel has focus; while collapsed those keys are ignored silently rather than acting as global shortcuts that invisibly change a later layout. Mouse resizing is not part of this interaction.
The queue always remains in the physical column when expanded, even on short terminals; it is not relocated under the library side as a responsive fallback.
Text should be laid out from the current frame width after each resize: text that no longer fits must be abbreviated or ellipsized, and text should expand again when there is room.
_Avoid_: using "left panel" without qualification when discussing #111 — the resize target is specifically the queue column, not the normal library table split or any modal/sidebar. Also avoid reusing `v` as a view toggle: there is only one view, and no view setting to toggle.
(Before #361, this column and its behavior belonged specifically to "Power View", a second, now-deleted UI called "Standard" being the contrast. That view is the only view now, so the qualifier is dropped throughout this glossary.)

**Panel focus**:
The restart-persistent focus choice between the queue side and the library side. It is separate from **Library position**: restoring the library side's focused item does not imply that the library side itself should have focus, and restoring queue-side focus does not change the saved library position.

**Toast**:
A transient status message rendered over the TUI for short-lived feedback, such as confirming an action or explaining why a keypress had no effect. Queue-column resize actions should use this existing feedback channel.
_Avoid_: introducing a separate notification surface for routine in-app feedback; use the existing toast/status mechanism.

**Audio visualizer**:
An optional TUI visualization of mbv's own playback audio, scoped to PulseAudio/PipeWire-pulse capture. When enabled, mbv must capture mbv-owned audio only, typically by routing mpv output through a dedicated Pulse-compatible sink and capturing that sink's monitor, but the routing must not introduce noticeable audio latency or A/V sync regression. If the first routing strategy causes latency, the implementation should troubleshoot and pivot to a cleaner capture path rather than accept degraded playback. The visualizer renders at the bottom of the library list and `v` enables/disables that embedded surface without moving focus or scroll; the preference may persist.
_Avoid_: promising generic ALSA support, capturing the default system monitor, treating visualizer failure as a normal user-facing state instead of a defect to diagnose from logs, exposing a second visualizer toggle in F2 settings, or making it a permanent ordinary tab. (Before #361 there was a second, fullscreen visualizer surface reachable "outside Power View"; that surface no longer exists — there is no outside. #98, which owns this feature, now scopes to one surface.)

**Audio visualization subsystem**:
The subsystem that owns visualizer audio routing, capture, analysis, and bar-state lifecycle. Playback owns mpv/session semantics, and UI owns rendering; the audio visualization subsystem sits between them and turns mbv-owned audio into renderable visualization state for the embedded surface. The capture/analysis pipeline runs only while the visualizer surface is visible. Synthetic audio input is part of the design toolbox: it should support routing/capture spikes and deterministic visualizer rendering tests.
