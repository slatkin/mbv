...
Among however many ctrl-socket clients are attached to a daemon simultaneously (already supported today — `ctrl_clients` broadcasts to a `Vec`, not a single peer), at most one holds driving authority at a time: its queue/session view is what the daemon treats as authoritative. Settled via #97's grilling: this is forced by the daemon's real deployment (one central server, one physical audio output), not an arbitrary design preference. Uses **takeover** semantics — a client's successfully executed control command bumps the previous driver to observer — rather than connection rejection, since the expected case is one person switching machines, not multiple parties contending for control. A received command that is rejected or ignored does not make that client the driver; driving authority changes only when the client's will has actually been exerted over daemon playback or queue state. "Driving" is authority over the daemon's queue/session, not proof that media is actively playing: a stopped daemon with a retained queue still has a driver, while a daemon with no queue/session has none. A successful Emby remote-control websocket command clears any ctrl-socket driver; the daemon may still have queue/session state afterward, but attached TUIs are observers until one successfully executes a later ctrl command.
