...
Its resize controls are `Ctrl+Alt+Left`/`Ctrl+Alt+Right`, active only while Power View itself is active; outside Power View those keys are ignored silently rather than acting as global shortcuts that invisibly change a later Power View layout from another tab.
