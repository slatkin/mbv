...
The left column in Power View: the column that contains the Power View queue card at the top and the queue below it. This is the "left panel" meant by #111's user-resizable panel-width work. Its width is stored as a persistent absolute column count, not a terminal-width percentage or ratio. The default and minimum width are both 40 columns, preserving today's layout until the user explicitly widens it; restored preferences above 60% of the current terminal width are normalized down and saved so the right-side main content remains usable, except that the 40-column minimum always wins on very narrow terminals.
