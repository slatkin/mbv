...
impl LocalPlaybackTarget {
    ...
    pub(in crate::app) fn adjust_volume(app: &mut App, delta: i64) {
        ...
        if active {
            ...
            app.ui_volume = u8::try_from((i64::from(app.ui_volume) + delta).clamp(0, 200))
                .expect("clamped volume fits in u8");
        }
        app.save_prefs();
    }
    ...
    pub(in crate::app) fn indicator_data(app: &App) -> Option<IndicatorData> {
        ...
        let res_str = if video_is_image || res_h == 0 {
            ...
            short_resolution_label(
                u64::try_from(res_h.max(0)).expect("nonnegative resolution height"),
            )
            .to_string()
        };
        let res_dim = res_str == "--";
