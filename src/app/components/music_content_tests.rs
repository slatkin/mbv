...
#[test]
fn visual_mode_toggles_artist_descendants_and_exits_without_status_wiring() {
    let mut owner = tree_owner(&[("Alpha", &["a-0", "a-1"]), ("Beta", &["b-0"])]);
    owner.expand_all_tree_roots();
    press(&mut owner, Key::Home);
    let root = owner.browser.selected_id().expect("artist root selected");
    ...
    assert_eq!(
        ...
        None,
        "Visual mode is a local tree mutation; row 4.3 owns status projection"
    );
    assert_eq!(
        owner.browser.marked_album_targets(),
        vec!["a-0".to_string(), "a-1".to_string()]
    );
    assert_eq!(
        owner.browser.mark_state(root),
        tui_treelistview::TreeMarkState::Marked
    );
    assert!(owner.browser.is_visual_mode());
    assert_eq!(
        owner.on_key_result(&KeyEvent::new(Key::Char(' '), KeyModifiers::NONE)),
        LeafKeyResult::Consumed(None)
    );
    assert!(!owner.browser.is_visual_mode());
    assert_eq!(
        owner.on_key_result(&KeyEvent::new(Key::Char('V'), KeyModifiers::SHIFT)),
        LeafKeyResult::Consumed(None)
    );
    assert!(owner.browser.is_visual_mode());

    assert_eq!(
        owner.on_key_result(&KeyEvent::new(Key::Esc, KeyModifiers::NONE)),
        LeafKeyResult::Consumed(None)
    );
    assert!(owner.browser.marked_album_targets().is_empty());
    assert!(!owner.browser.is_visual_mode());
}

#[test]
fn modified_tree_clicks_toggle_and_range_only_album_membership() {
    let mut owner = tree_owner(&[("Alpha", &["a-0", "a-1"]), ("Beta", &["b-0"])]);
    owner.expand_all_tree_roots();
    let area = Rect::new(0, 0, 48, 8);
    paint_tree(&mut owner, area);
    ...
    let album_0_at = tree_point(&owner, area, album_0);
    let album_1_at = tree_point(&owner, area, album_1);
    let beta_album_at = tree_point(&owner, area, beta_album);
    owner.on_slot_event(LibrarySlotEvent::List(MediaListSurfaceInput::Click(
        album_0_at,
    )));
    ...
    // Shift range crosses the Beta root row but emits no root identity.
    owner.on_slot_event(LibrarySlotEvent::List(MediaListSurfaceInput::RangeClick(
        beta_album_at,
    )));
    assert_eq!(
        owner.browser.marked_album_targets(),
...
#[test]
fn tree_marks_do_not_touch_the_music_workspace_selection() {
    let mut owner = tree_owner_with_tracks(
        &[("Alpha", &["a-0", "a-1"])],
        Some(vec![
            make_item("track-0", "Audio"),
            make_item("track-1", "Audio"),
        ]),
    );
    owner.track_list.select_last();
    owner.track_list.toggle_selection(&"track-1".to_string());
    let workspace_selection = owner.track_list.multi_selection().to_vec();

    owner.expand_all_tree_roots();
    press(&mut owner, Key::Home);
    owner
        .browser
        .toggle_node(owner.browser.selected_id().expect("root"));

    assert_eq!(
        owner.track_list.multi_selection(),
        workspace_selection.as_slice()
    );
    assert_eq!(
        owner.browser.marked_album_targets(),
        vec!["a-0".to_string(), "a-1".to_string()]
