...
mod leaf_key_tests {
    ...
    fn leaf_disposition_conversion_distinguishes_claims() {
        ...
        let request = Msg::Shell(Box::new(ShellRequest::LibraryRoutesEnter));
        assert_eq!(
            LeafKeyResult::Consumed(Some(Box::new(request.clone()))).into_option(),
            Some(request)
        );
    }
}
    ...
    pub(crate) fn into_option(self) -> Option<Msg> {
        match self {
            ...
            Self::Consumed(message) => message
                .map(|message| *message)
                .or(Some(Msg::TerminalEvent(TerminalObserverEvent::KeyClaimed))),
        }
    }
}
