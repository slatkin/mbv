...
fn abs_book_hero_uses_isolated_book_cover_key() {
    ...
    let generic_key =
        crate::app::images::audiobookshelf_cover_cache_key(server, "book-hero-isolation", suffix);
    assert!(
...
fn abs_book_stays_mounted_and_preserves_selection_across_switch() {
    ...
    app.audiobookshelf_browse.push(
        crate::app::types_audiobookshelf_browse::AudiobookshelfBrowseState::new(podcast_library),
    );
    app.tab = TabSelection::AudiobookshelfLibrary(0);
    app.panel_focus = PanelFocus::Library;
    let mut model = Model::new(app);
