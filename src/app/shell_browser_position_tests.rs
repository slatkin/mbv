...
/// Task 6.1: the Movies/HomeVideos/Generic destination is the embedded
/// `BrowserContent` owner inside the mounted `LibraryPanel` (no mounted
/// component), so "mounts and syncs" means the owner is installed and its
/// local movement still routes through the same typed browser request.
#[test]
fn shell_mounts_and_syncs_the_generic_emby_browser() {
    let mut model = Model::new(make_movie_app());
    model.sync_mounted_surfaces();
    // The mounted component does not exist for these kinds any more.
    assert_eq!(model.emby_browser_id, None);

    let message = drive_owner_key(&mut model, Key::Down, KeyModifiers::NONE);
    // Down routes through the typed browser request.
    let Some(Msg::Shell(ShellRequest::BrowserCursorIndex { index })) = message else {
        panic!("owner movement should emit the typed index request");
    };
...
/// the focused surface returns typed `BrowserCursorIndex` requests in place
/// of the raw legacy key, and the shell derives the active Emby library
/// index from its own tab state and runs the same `App` cursor methods the
/// legacy `handle_lib_key` movement arms call. The App cursor must move
/// through that typed path (never a raw cursor-field write). Task 5.8
/// deleted the Grid presentation as unreachable (design D13), so every
/// painted list is one-column: Down strides one item, Home/End jump to the
/// first/last item, and Left/Right/h/l stay unbound locally (raw key
/// consumed without movement, App cursor unchanged). Task 6.1: the surface
/// is the embedded `BrowserContent` owner behind the focused
/// `LibraryPanel`.
#[test]
fn shell_emby_browser_movement_drives_app_cursor_via_typed_requests() {
    ...
    // Paint the panel at 150 columns so the Wide skeleton establishes the
    // shared owner's painted flow (the generic library never takes a
    // special rail).
    draw_owner_model(&mut model, 150, 40);
    model.sync_mounted_surfaces();

    // Down: the focused surface returns a typed `BrowserCursorIndex`
    // (one selectable row, in place of the raw key), and the shell applies
    // it — the one-column list strides the App cursor one item, exactly
    // like the legacy arm.
    let Some(Msg::Shell(ShellRequest::BrowserCursorIndex { index })) =
        drive_owner_key(&mut model, Key::Down, KeyModifiers::NONE)
    else {
        panic!("focused owner Down must emit BrowserCursorIndex, got no typed request");
    };
    ...
    assert_eq!(
        ...
        1,
        "Down must apply the owner-resolved index"
    );
    assert!(
    ...
    assert_eq!(
        ...
        1,
        "owner cursor remains locally resolved"
    );

    // End/Home jump the App cursor to the last/first item through
    // `App::jump_lib_cursor`.
    let Some(Msg::Shell(ShellRequest::BrowserCursorIndex { index })) =
        drive_owner_key(&mut model, Key::End, KeyModifiers::NONE)
    else {
        panic!("focused owner End must emit BrowserCursorIndex, got no typed request");
    };
    assert_eq!(index, 9, "End must resolve to the last item");
    model.handle_browser_request(ShellRequest::BrowserCursorIndex { index });
    assert_eq!(model.app.libs[0].nav_stack[0].resting().cursor(), 9);
    let Some(Msg::Shell(ShellRequest::BrowserCursorIndex { index })) =
        drive_owner_key(&mut model, Key::Home, KeyModifiers::NONE)
    else {
        panic!("focused owner Home must emit BrowserCursorIndex, got no typed request");
    };
    ...
    // leaving the owner and App cursors unchanged.
    model.app.panel_mode = PanelMode::Both;
    draw_owner_model(&mut model, 100, 40);
    model.sync_mounted_surfaces();
    for key in [Key::Left, Key::Right, Key::Char('h'), Key::Char('l')] {
        assert_eq!(
            drive_owner_key(&mut model, key, KeyModifiers::NONE),
            None,
    ...
    assert_eq!(
        browser_owner(&model).cursor(),
        0,
        "unbound Left/Right/h/l must not move the owner cursor"
    );
    assert_eq!(
        model.app.libs[0].nav_stack[0].resting().cursor(),
        0,
    ...
    let Some(Msg::Shell(ShellRequest::BrowserCursorIndex { index })) =
        ...
        panic!("focused owner Down must still emit BrowserCursorIndex, got no typed request");
    };
    assert_eq!(index, 1);
    model.handle_browser_request(ShellRequest::BrowserCursorIndex { index });
    assert_eq!(
...
fn teardown_flush_captures_live_browser_scroll_without_navigation() {
    ...
    model.sync_emby_browser();
    model.sync_active_destination();
    // Seed the shared owner's position directly (the owner's local state is
    // the only cursor store).
    browser_owner_mut(&mut model).apply_position(0, 6);
