...
impl App {
    ...
    pub(super) fn activate_service_entry(&mut self) {
        ...
        let intent = match entry {
            ...
            ServiceEntry::Audiobookshelf => ServiceActionIntent::SetupAudiobookshelf,
    ...
    fn route_service_action(&mut self, intent: ServiceActionIntent) {
        match intent {
            ...
            ServiceActionIntent::SetupAudiobookshelf => self.flash(
                "Audiobookshelf setup is not available yet".into(),
    ...
    pub(super) fn service_state_label(&self, entry: ServiceEntry) -> &'static str {
        match entry {
            ...
            // There is deliberately no Audiobookshelf runtime yet. Not configured
            // is truthful; showing Ready would imply an implementation we do not have.
            ServiceEntry::Audiobookshelf => "Not configured",
    ...
    pub(super) fn service_action_label(&self, entry: ServiceEntry) -> &'static str {
        match entry {
            ...
            ServiceEntry::Audiobookshelf => "Setup unavailable (coming later)",
