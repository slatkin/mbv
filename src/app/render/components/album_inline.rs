...
impl App {
    pub(in crate::app::render) fn render_grouped_album_rows_inline_plan(
        ...
        let hero_rows =
            (album_hero_detail_rows(self.images_enabled()) + HERO_BLOCK_EXTRA_ROWS as usize) as u16;
        ...
        if let Some(hero_area) = hero_area {
            ...
            let content_area = library::selected_detail_content_area(
                hero_area,
                super::list_rows::SELECTED_BLOCK_SIDE_PADDING,
                HERO_BLOCK_EXTRA_ROWS,
            );
            if let Some(album) = albums.get(cursor) {
                self.render_album_hero_detail(f, content_area, album, focused);
            }
        }
