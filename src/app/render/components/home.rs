...
pub(in crate::app) fn render_home_content(
    ...
    let list_area = if let Some(list_panel) = green_panel_full {
        let panel_bg = palette::surface_colors(palette::Surface::LibraryPanel, focused).fill;
        f.render_widget(
    ...
    if spacer_area.y < area.bottom() && spacer_area.width > 0 {
        let panel_bg = palette::surface_colors(palette::Surface::PillRowGap, false).fill;
        f.render_widget(
