...
pub(in crate::app::render) struct AlbumRowCtx<'a> {
    pub row_area: Rect,
    pub idx: usize,
...
impl App {
    pub(in crate::app::render) fn render_artist_header_row(
        &self,
        f: &mut Frame,
        row_area: Rect,
        ...
        let label_style = Style::default().fg(palette::TEXT_FOCUS_ACCENT);
        let mut spans = Vec::with_capacity(3);
        if grouped_block {
            spans.push(super::list_rows::selection_marker(
                false,
    ...
    pub(in crate::app::render) fn render_album_row(&self, f: &mut Frame, ctx: AlbumRowCtx) {
        let AlbumRowCtx {
            row_area,
            idx,
            album_info,
