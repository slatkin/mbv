...
impl App {
    pub(in crate::app::render) fn render_feed_home_video_group_view(
        &mut self,
        f: &mut Frame,
        area: Rect,
        lib_idx: usize,
        ...
        if row < max_y {
            ...
            selector_tabs = crate::app::render::render_pill_bar(
                f,
                Rect {
                    x: area.x,
                    y: row,
        ...
        let selected_panel_width = text_w_with_sb.saturating_sub(
            2 * crate::app::render::components::list_rows::SELECTED_BLOCK_SIDE_PADDING as usize,
        ) as u16;
        ...
        let text_w = crate::app::render::content_width(list_area.width, needs_scrollbar);
        ...
        if needs_scrollbar && focused {
            crate::app::render::render_right_scrollbar_with_viewport(
                f,
                list_area,
                items.len(),
                visible_items.max(1),
