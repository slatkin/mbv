    ...
    pub(super) fn render_library_table(&mut self, f: &mut Frame, area: Rect, lib_idx: usize) {
        ...
        let (display_items, cursor): (Vec<(usize, crate::api::MediaItem)>, usize) = {
            ...
            if let Some(s) = &lib.search {
                let items: Vec<(usize, crate::api::MediaItem)> = s
                    .results
                    .iter()
                ...
                let items: Vec<(usize, crate::api::MediaItem)> = lvl
                    .map(|l| {
                        l.items
                            .iter()
                            .enumerate()
        ...
        let lib_episode_img_h: u16 = self
            .image_picker
            .as_ref()
            .map(|p| {
                let fs = p.font_size();
        ...
        let all_heights: Vec<u16> = display_items
            ...
            .map(|(i, (_, item))| {
                let is_audio = item.media_type == "Audio" || item.item_type == "Audio";
                let base: u16 = if item.is_folder
                    ...
                    && item.item_type != "Season"
                {
                    ...
                    let img_h_for_layout = if actual_sel_img_h > 0 {
                        actual_sel_img_h
                    } else {
                        sel_img_h
                    };
        ...
        for (vi, (_, item)) in display_items[scroll..].iter().enumerate() {
            ...
            let img_actual = if show_img {
                if let Some(Some(state)) = self.card_image_states.get_mut(&cache_key) {
                    let (img_w, img_h) = if is_audio || is_album_folder {
                        ...
                        || (is_feed_lib && item.item_type == "Video")
                    {
                        (LIB_EPISODE_IMG_W, LIB_EPISODE_IMG_H)
                    } else {
                        (LIB_SELECTED_IMG_W, LIB_SELECTED_IMG_H)
            ...
            let meta_line: Line = if is_episode_like && item.item_type != "Episode" {
                ...
                match item.item_type.as_str() {
                    ...
                    _ => {
                        ...
                        {
                            let pct = (item.playback_position_ticks * 100
                                / item.runtime_ticks.max(1))
                                as u64;
                            spans.push(Span::styled(
                                format!("  {pct}%"),
                                Style::default().fg(palette::YELLOW),
            ...
            let overview_lines: Vec<String> =
                if !is_audio && selected && images_enabled && !item.overview.is_empty() {
                    let w = content_w.max(1);
                    wrap(&trunc_overview(&item.overview), w)
                        .into_iter()
                        .map(|s| s.into_owned())
                        .collect()
                } else {
            ...
            let base_lines = if is_album_folder {
                2
            ...
            let dir_lines: usize =
                if selected && item.item_type == "Movie" && !item.director.is_empty() {
                    2
                } else {
                    0
                };
            ...
            let constraints: Vec<Constraint> =
                (0..line_count).map(|_| Constraint::Length(1)).collect();
