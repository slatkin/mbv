...
//! | `WideSplitGutter` | — | pinned nowhere at buffer level (see below) |
...
//! | `ContextMenuSelectedRow` | — | pinned nowhere at buffer level (see below) |
...
//! | `InlineHero` | — | pinned nowhere at buffer level (see below) |
...
//! | `SidebarBody` | — | pinned nowhere at buffer level (see below) |
...
//! | `StatusBarPill` | — | pinned nowhere at buffer level (see below) |
...
//! | `SidebarBand` / `NonHeroSidebarBand` | — | pinned nowhere at buffer level (see below) |
//! | `TabBar` | yes (Both, LibraryOnly, mini library) | `tests.rs` tab-bar suites |
//! | `PopupFrame` / `PopupDimBackdrop` | — | pinned nowhere at buffer level (see below) |
//!
//! The surfaces pinned **nowhere at buffer level** in this tree, recorded as
//! an honest gap rather than hidden: `WideSplitGutter` and `QueueCardVisualizer`
//! (their boundary/inset painters paint from shell-derived or component-local
//! rects no frame or component test asserts), the sidebar body/band rows
//! (`SidebarBody`, `NonHeroSidebarBody`, `SidebarBand`, `NonHeroSidebarBand`),
//! `InlineHero`, `PlaybackStatusPill`, `StatusBarPill`, `ContextMenuSelectedRow`
//! and the popup pair (`PopupFrame`, `PopupDimBackdrop`) — none of this tree's
//! pre-existing tests asserts the painted fill of those regions, and the
//! neutrality rule forbids editing the pre-existing test files to add one.
//! Their painters are still guarded by the ast-grep rules; only the rendered
...
/// this module (see the module coverage table) or explicitly accounted for.
/// This test fails if a new `Surface` variant is added without the module
/// documentation being revisited, so a silent, unpinned surface cannot appear.
#[test]
