...
//! design D1/D2/D7).
//!
//! [`Surface`] is generated by `declare_surfaces!` from one variant list, so a
//! new variant cannot be added without the table's `row` failing to compile
//! until it is placed. [`Level`] is D2's nesting level; [`FocusSource`] records which
...
//!
//! Only [`Surface`] and the resolver's `surface_colors` are
...
/// cannot be added without the table's `row` failing to compile until it is
/// placed.
