...
#[test]
fn narrow_grouped_music_hero_uses_shared_contained_presentation() {
    // Task 3.6a: the selected-album hero paints its title two rows below the
    // card's top border (shared narrow presentation via `paint_hero_content`)
    // with the standard selected role. Before the fix the bespoke path in
    // `album_inline.rs` painted a BOLD title at the card's top row.
    let mut app = make_music_group_app();
    ...
    terminal
        .draw(|f| {
            crate::app::render::components::album::render_grouped_album_rows(
                ...
                0,
                &albums,
                super::components::album::AlbumRowsCursorCtx {
                    cursor: 0,
                    stored_scroll: 0,
                },
                true,
    ...
    assert!(hero.height > 3, "hero card must be reserved: {hero:?}");
    let buffer = terminal.backend().buffer();
    let pad = super::components::list_rows::SELECTED_BLOCK_SIDE_PADDING;
    assert_eq!(
        buffer[(hero.x + pad, hero.y + 2)].symbol(),
        "F",
        "hero title must start two rows below the card's top border"
    );
    for dx in 0..("First Album".len() as u16) {
        assert!(
            !buffer[(hero.x + pad + dx, hero.y + 2)]
                .style()
                .add_modifier
                .contains(ratatui::style::Modifier::BOLD),
            "hero title must use the standard selected role, not BOLD"
        );
    }
}
