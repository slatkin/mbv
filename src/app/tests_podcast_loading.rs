...
fn unfetched_podcast_app(shows: usize) -> App {
    ...
    app.config.lock().unwrap().audiobookshelf_setup =
        Some(AudiobookshelfSetup::new(String::new()));
...
fn start_detail_without_service_setup_does_not_leak_the_in_flight_mark() {
    ...
    app.audiobookshelf_libraries.push(mbv_core::audiobookshelf::AudiobookshelfLibrary {
        id: "abs-podcasts".into(),
        name: "ABS Podcasts".into(),
        media_type: "podcast".into(),
    });
...
fn tab_activation_rerequests_the_active_pills_required_shows() {
    ...
    for i in 0..3 {
        app.audiobookshelf_browse[0]
            .cache_detail(format!("show-{i}"), Vec::new());
    }

    app.activate_audiobookshelf_position(0);

    let state = &app.audiobookshelf_browse[0];
    assert!(state.detail_cache.is_empty(), "cached episodes are replaced");
    assert_eq!(
        loading(state),
    ...
    for i in 0..3 {
        app.audiobookshelf_browse[0]
            .cache_detail(format!("show-{i}"), vec![]);
    }
