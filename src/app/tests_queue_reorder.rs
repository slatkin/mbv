...
// ── P2 fix: pending remote move reconciliation ───────────────────────────

#[test]
fn unrelated_queue_update_does_not_consume_pending_move_cursor() {
    // P2 scenario: a remote move is pending, but a QueueUpdated event
    // arrives with a different queue arrangement (e.g., another client
    // modified the queue). The pending cursor must NOT be consumed.
    let _guard = crate::config::TestStateDirGuard::new();
    let local_items = make_items(2);
    let remote_items = make_items(3);
    let (mut app, _cmd_rx) =
    ...
    // Move item at cursor 1 (id1) up to position 0
    app.move_queue_item_up();
    // Pending: item_id="id1", target_idx=0

    // An unrelated QueueUpdated arrives with a DIFFERENT arrangement
    // (e.g., another client prepended a new item). id1 is NOT at position 0.
    let mut unrelated_items = make_items(4);
    unrelated_items[0].id = "new-item".into();
    app.handle_player_event(PlayerEvent::QueueUpdated {
        items: unrelated_items.clone(),
        cursor: 3,
        source: crate::config::QueueSource::Remote,
    });

    // Pending move should NOT have been consumed — cursor should use the
    // event's cursor (3), not the pending target (0).
    assert_eq!(
        app.remote_player_tab.as_ref().unwrap().queue_cursor,
        3,
        "unrelated QueueUpdated must not consume the pending move cursor"
    );
}

#[test]
fn matching_queue_update_consumes_pending_move_cursor() {
    // The happy path: QueueUpdated arrives with the moved item at the
    // expected position, confirming the move was applied.
    let _guard = crate::config::TestStateDirGuard::new();
    let local_items = make_items(2);
    let remote_items = make_items(3);
    let (mut app, _cmd_rx) =
    ...
    // QueueUpdated arrives reflecting the move: id1 is at position 0
    app.handle_player_event(PlayerEvent::QueueUpdated {
        items: vec![
            remote_items[1].clone(),
            remote_items[0].clone(),
        ...
        cursor: 1, // event says cursor=1, but we override to 0
        source: crate::config::QueueSource::Remote,
    });
    ...
    assert_eq!(
        app.remote_player_tab.as_ref().unwrap().queue_cursor,
        0,
        "matching QueueUpdated must consume the pending move and use the optimistic cursor"
    );
}
