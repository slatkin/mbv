...
fn wide_podcast_surface_snapshot() {
    ...
    // Task 3.8 removes the empty placeholder hero reservation: the browse
    // body starts immediately below the header instead of being pushed down.
    for row in ["Show 0", "Show 1", "Show 2", "Show 3", "Show 4"] {
        assert_eq!(
            output.matches(row).count(),
            1,
    ...
    let first_row = output
        .lines()
        .position(|line| line.contains("Show 0"))
        .expect("wide podcast browse body must be visible");
    assert!(
        first_row < 10,
        "wide podcast body must not reserve placeholder hero rows (line {first_row})"
    );
}
