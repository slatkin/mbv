use super::types_player_tab::PlayerTab;
use mbv_core::api::MediaItem;
use mbv_core::playback_queue::PlaybackQueue;

pub(super) struct LocalDaemonBootstrap {
    pub(super) player_tab: PlayerTab,
    pub(super) queue_source: crate::config::QueueSource,
    pub(super) last_played_item_id: Option<String>,
    pub(super) last_played_completed: bool,
    pub(super) adopt_queue: Option<(Vec<MediaItem>, usize, crate::config::QueueSource)>,
    pub(super) positions: std::collections::HashMap<String, i64>,
}

pub(super) fn bootstrap_local_daemon_queue(
    remote_items: Vec<MediaItem>,
    remote_cursor: usize,
    remote_source: crate::config::QueueSource,
    saved_state: Option<crate::config::QueueState>,
) -> LocalDaemonBootstrap {
    if !remote_items.is_empty() {
        return LocalDaemonBootstrap {
            player_tab: PlayerTab::new(remote_items, remote_cursor),
            queue_source: remote_source,
            last_played_item_id: None,
            last_played_completed: false,
            adopt_queue: None,
            positions: Default::default(),
        };
    }

    let Some(state) = saved_state.filter(|state| !state.items.is_empty()) else {
        return LocalDaemonBootstrap {
            player_tab: PlayerTab::new(remote_items, remote_cursor),
            queue_source: remote_source,
            last_played_item_id: None,
            last_played_completed: false,
            adopt_queue: None,
            positions: Default::default(),
        };
    };

    let (player_tab, adopt_items, adopt_cursor) = if state.slot_ids.is_some() {
        let pq = PlaybackQueue::from_persisted(
            state.items.clone(),
            state.slot_ids.clone(),
            state.active_slot_id,
            state.revision,
            state.next_slot_id,
        );
        let cursor = pq.current_index().unwrap_or(0);
        let items = pq.items_snapshot();
        let slot_ids = pq.slot_ids();
        let active_slot_id = pq.active_slot_id();
        let revision = pq.revision();
        let tab = PlayerTab::from_slot_snapshot(
            items.clone(),
            slot_ids,
            active_slot_id,
            revision,
            cursor,
        );
        (tab, items, cursor)
    } else {
        let cursor = super::actions::queue_restore_cursor(
            &state.items,
            state.cursor,
            state.last_played_item_id.as_deref(),
            state.last_played_completed,
        );
        let items = state.items.clone();
        let tab = PlayerTab::new(items.clone(), cursor);
        (tab, items, cursor)
    };
    LocalDaemonBootstrap {
        player_tab,
        queue_source: state.source.clone(),
        last_played_item_id: state.last_played_item_id.clone(),
        last_played_completed: state.last_played_completed,
        adopt_queue: Some((adopt_items, adopt_cursor, state.source)),
        positions: state.positions,
    }
}
