...
impl App {
    ...
    fn cycle_sub(&mut self) {
        if let Some(ref _conn_id) = self.connected_session_id.clone() {
            // Subtitle selection is per-client; just toggle off/on
            self.toggle_sub();
            return;
        }
        let (tracks, current_id) = {
    ...
    fn render_playback_controls(&mut self, f: &mut ratatui::Frame, area: Rect) {
        ...
        const BTNS_W: u16 = 30; // 6 buttons × 5 cells each
        ...
        // Row 1 — elapsed (left), buttons (center, nf mode only), total (right)
        ...
        if self.use_nerd_fonts {
            ...
            let pp_icon = if !paused { "\u{F03E4}" } else { "\u{F040A}" }; // nf-md-pause / nf-md-play
    ...
    fn render_playlist_cards(&mut self, f: &mut ratatui::Frame, area: Rect) {
        ...
        if self.terminal_height > 34 {
            self.render_playlist_filmstrip(f, area, cursor, active, active_idx,
                FILMSTRIP_STRIP_H);
            return;
        }
    ...
    fn render_playlist_filmstrip(
        ...
        // Center the card+strip block as one unit vertically.
        ...
        // Active item always in the center slot of the strip.
        ...
        for i in 0..n_fit {
            ...
            if item_idx == cursor {
                // Green border for the active strip item; render card borderless inside.
                let block = Block::default()
                    .borders(Borders::ALL)
                    .border_type(BorderType::Rounded)
        ...
        // ◀/▶ arrows flanking the strip, shown when there are items off-screen.
    ...
    fn render_playlist_presentation(&mut self, f: &mut ratatui::Frame, area: Rect) {
        ...
        // Sticky playback buttons: shown only when use_nerd_fonts is enabled.
