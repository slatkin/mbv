...
### Daemon visualizer Snapclient

For headless spectrum streaming, enable `[mbvd].audio_pipe_enabled`, install both `cava` and `snapclient`, and configure the dedicated visualizer client if Snapserver is not local. mbvd defaults to Snapserver `127.0.0.1:1704`, Snapclient host ID `puffin-balls`, instance `2`, and FIFO `/tmp/mbv-spectrum.fifo`. Verify that `puffin-balls` appears in Snapserver's web UI and assign it to the same stream as the audible client before enabling the visualizer. The daemon's second Snapclient is stopped with visualization and never replaces or controls the primary hardware-output client.

For a deployment check, start a known track, enable the visualizer, and compare the first bar response with the audible transient; repeat after playback has been running long enough to fill the Snapcast buffer. The visualizer should trail the audible output by the local CAVA/Snapclient scheduling delay, never lead it by a full Snapcast buffer. Then disable visualization, stop playback, disconnect the control client, and restart `mbvd`; `/tmp/mbv-spectrum.fifo` and the private CAVA resources should disappear while the primary Snapclient remains connected. To exercise failure handling without changing primary playback, temporarily stop only the dedicated `puffin-balls#2` client or make CAVA unavailable, confirm a `SpectrumFailed` event, and restore the prerequisite before the next start.
